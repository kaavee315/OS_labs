...
{	
	keep_going = 0;
	if(killpg(pgid_fore,SIGINT)==1){
		pid_t    pid;
		int      stat;
		waitpid(-pgid_fore,0,0);
		// while ( (pid = waitid(P_PGID,0, &stat,WNOHANG)) > 0);	
	}
    return;
}
		...
		{	
			if(no_tokens==2) {
					...
					if(server_set!=1) {
						...
						{	pgid_fore=pid;
							if(setpgid(pid,0)<0) {
								printf("Error in fore0");
							}
						}
						else if(setpgid(pid,pgid_fore)<0) {
							pgid_fore=pid;
							if(setpgid(pid,0)<0) {
								printf("Error in fore1");
							}
						}
						waitpid(pid,0,NULL);
					}
				...
				{	
					...
					else if(setpgid(pid1,pgid_fore)<0) {
						pgid_fore=pid1;
						if(setpgid(pid1,0)<0) {
							printf("Error in fore1");
						}
					...
					else if(setpgid(pid2,pgid_fore)<0) {
						pgid_fore=pid2;
						if(setpgid(pid2,0)<0) {
							printf("Error in fore1");
						}
		...
		{	if(no_tokens<2) {
				...
				if(server_set!=1) {
					...
					for(j=0;j<no_tokens-1;j++) {
						if(pgid_fore<0)
						{	pgid_fore=pids[j];
							if(setpgid(pids[j],0)<0) {
								printf("Error in fore0");
							}
						}
						else if(setpgid(pids[j],pgid_fore)<0) {
						pgid_fore=pids[j];
						if(setpgid(pids[j],0)<0) {
							printf("Error in fore1");
						}
...
//getbg filename
		...
		{	if(no_tokens!=2) {
				...
				if(server_set!=1) {
					...
					{	pgid_back=pid;
						if(setpgid(pid,0)<0) {
							printf("Error in back0");
						}
					}
					else if(setpgid(pid,pgid_back)<0) {
					pgid_back=pid;
					if(setpgid(pid,0)<0) {
						printf("Error in fore1");
					}
